...
export function auditView(f, p, role, purpose) {
  ...
  f.authorize(p, [`audit_${role}`, ...(role === 'finance' ? ['operator'] : role === 'security' ? ['security'] : role === 'technical' ? ['auditor'] : [])]);
  return f.transaction(p, now => {
    ...
    return signed({ format: 'IF-AUDIT-PROJECTION-1', tenant_id: p.tenant_id, role, issued_at: now, entries, truncated: rows.length === 1000, full_chain: false }, f.keys(p.tenant_id).audit, 'audit-projection');
  });
}
