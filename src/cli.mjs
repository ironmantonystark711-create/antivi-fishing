...
try {
  if (command === 'init') {
    ...
    requireThat(['action-approval', 'evidence', 'root-policy', 'batch-approval', 'runtime-config', 'emergency-policy', 'coverage-test', 'release-build', 'release-review', 'release-observation', 'deployment-rollback'].includes(purpose), 'INV-400-SCHEMA', 'Unsupported signing purpose');
    ...
    console.log('Usage:\n  node src/cli.mjs init --dir ./var/local [--tenants acme,globex]\n  node src/cli.mjs serve --dir ./var/local [--port 8080] [--origin http://127.0.0.1:8080]\n  node src/cli.mjs sign --key PATH --input PATH --output PATH [--purpose action-approval|evidence|root-policy|batch-approval|release-build|release-review|release-observation|deployment-rollback]');
