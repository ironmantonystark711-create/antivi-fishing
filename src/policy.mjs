...
export class PolicyPromotions {
  ...
  challenge(p, input) {
    this.f.authorize(p, ['policy_admin', 'security']); fields(input, ['candidate', 'reviewed_commit', 'stage', 'simulation_id', 'expires_at']);
    validatePolicy(input.candidate); text(input.reviewed_commit, 'reviewed commit', 64); identifier(input.simulation_id, 'simulation id'); oneOf(input.stage, STAGES, 'promotion stage');
  ...
  promote(p, input) {
    ...
    return this.f.transaction(p, now => {
      ...
      fields(x, ['format', 'tenant_id', 'policy_id', 'policy_version', 'policy_digest', 'baseline_digest', 'reviewed_commit', 'stage', 'simulation_id', 'simulation_digest', 'issued_at', 'expires_at']);
  ...
  current(tenant, policyDigest, stage, now = this.f.clock()) {
    ...
    fields(payload, ['format', 'tenant_id', 'policy_id', 'policy_version', 'policy_digest', 'baseline_digest', 'reviewed_commit', 'stage', 'simulation_id', 'simulation_digest', 'issued_at', 'expires_at', 'signer_set', 'approval_digests', 'promoted_at']);
