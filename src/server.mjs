...
export function createServer(fabric, { port = 8080, host = '127.0.0.1', origin = `http://127.0.0.1:${port}` } = {}) {
  ...
  const server = http.createServer({ maxHeaderSize: 16384 }, async (req, res) => {
    ...
    try {
      ...
      if (path === '/v1/keys/prepare' && req.method === 'POST') return send(201, fabric.keyGovernance.prepare(p, await body(req)));
      if (path === '/v1/keys/activate' && req.method === 'POST') return send(200, fabric.keyGovernance.activate(p, await body(req)));
      if (path === '/v1/keys/history' && req.method === 'GET') return send(200, fabric.keyGovernance.publicHistory(p));
      ...
      if (path === '/v1/coverage/revalidation-tasks' && req.method === 'GET') { fabric.authorize(p, ['security']); return send(200, fabric.store.list(p.tenant_id, 'coverage-task', 10000)); }
      ...
      if (path === '/v1/connectors/manifest' && req.method === 'GET') return send(200, fabric.connectorManifest(p));
