...
export class RuntimeGate {
  ...
  issue(principal, input) {
    ...
    return this.f.transaction(principal, now => {
      ...
      const payload = { ...clone(input), capability_id: randomUUID(), tenant_id: t, subject_id: principal.subject_id, hardware_backed: identity.hardware_backed, policy_digest: digest(policy), policy_version: policy.version, issued_at: now, expires_at: Math.min(now + input.ttl_ms, policy.expires_at), gate_id: this.f.config.gate_id, runtime_policy: clone(r) };
