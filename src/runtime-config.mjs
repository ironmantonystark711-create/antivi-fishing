...
export function verifyRuntimeConfiguration(snapshot, identities, tenant, gate, now) {
  ...
  integer(c.issued_at, 'configuration issue time', 1, now); integer(c.expires_at, 'configuration expiry', now + 1, c.issued_at + 7 * 86400000);
  requireThat(snapshot.signatures.length >= 3 && snapshot.signatures.length <= 5, 'INV-503-CONFIG', 'Bounded customer configuration quorum required', 503);
  ...
  for (const envelope of snapshot.signatures) {
    ...
    requireThat(!signers.has(envelope.protected.key_id) && typeof who.failure_domain === 'string' && who.failure_domain.length > 0, 'INV-503-CONFIG', 'Duplicate or unidentified configuration custodian', 503);
...
export class RuntimeIntegrity {
  ...
  check(tenant) {
    ...
    try {
      const fingerprint = digest(snapshot), prior = this.accepted.get(tenant), durable = f.store.get(tenant, 'runtime-integrity', f.config.gate_id);
      requireThat(!durable || (snapshot.config.version >= durable.version && (snapshot.config.version > durable.version || fingerprint === durable.digest)), 'INV-503-CONFIG', 'Persisted runtime configuration rollback rejected', 503);
      requireThat(!prior || now >= prior.last_check, 'INV-503-TIME', 'Runtime integrity clock regressed', 503);
      ...
      if (prior?.digest === fingerprint && prior.signers === signersDigest && now < snapshot.config.expires_at) { if (now >= prior.next_check) prior.next_check = now + snapshot.config.reload_interval_ms; prior.last_check = now; return snapshot.config; }
      ...
      f.store.tx(() => {
        f.store.clock(now);
        const latest = f.store.get(tenant, 'runtime-integrity', f.config.gate_id);
        requireThat(!latest || (config.version >= latest.version && (config.version > latest.version || fingerprint === latest.digest)), 'INV-503-CONFIG', 'Concurrent configuration rollback rejected', 503);
        if (!latest || latest.digest !== fingerprint) {
          f.store.put(tenant, 'runtime-integrity', f.config.gate_id, { version: config.version, digest: fingerprint, accepted_at: now }, now);
          f.store.audit(tenant, 'RUNTIME_CONFIGURATION_ACCEPTED', 'integrity-monitor', f.config.gate_id, { version: config.version, snapshot_digest: fingerprint }, now);
        }
      });
      this.accepted.set(tenant, { digest: fingerprint, version: config.version, next_check: now + config.reload_interval_ms, last_check: now, signers: signersDigest }); return config;
    } catch (e) {
  ...
  reload(principal, snapshot) {
    ...
    requireThat(config.version > Math.max(this.accepted.get(t)?.version ?? 0, this.f.store.get(t, 'runtime-integrity', this.f.config.gate_id)?.version ?? 0), 'INV-503-CONFIG', 'Signed reload must advance version', 503);
    if (this.hasFile) {
      const disk = parseStrict(readFileSync(this.path, 'utf8')); disk.tenants[t].runtime_snapshot = clone(snapshot);
