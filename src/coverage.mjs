...
export class CoverageLifecycle {
  ...
  revalidate(p, input) {
    ...
    return this.f.transaction(p, now => {
      ...
      requireThat(Array.isArray(e.permissions) && e.permissions.length > 0 && e.permissions.every(x => ['read', 'exact-mutation'].includes(x)) && Array.isArray(e.negative_tests) && ['no-certificate', 'wrong-tenant', 'state-race', 'replay', 'direct-bypass'].every(name => e.negative_tests.some(x => x.name === name && x.rejected === true && /^[a-f0-9]{64}$/.test(x.result_digest))), 'INV-412-COVERAGE', 'All assigned bypass tests and least-privilege evidence must pass', 412);
  ...
  refresh(p) {
    return this.f.transaction(p, now => { let stale = 0; for (const r of this.f.store.list(p.tenant_id, 'coverage', 10000)) if (r.status === 'ENFORCED' && (now - r.evidence_at > r.max_age_ms || r.evidence_expires_at <= now)) { r.status = 'UNKNOWN'; this.f.store.put(p.tenant_id, 'coverage', r.path_id, r, now); this.history(p.tenant_id, r, now, 'STALE_EVIDENCE'); this.f.store.put(p.tenant_id, 'coverage-task', r.path_id, { path_id: r.path_id, owner: r.owner, reason: 'REVALIDATION_REQUIRED', created_at: now }, now); stale++; } return { stale }; });
  }
}
