...
try {
 ...
 writeFileSync('reports/browser/results.json', JSON.stringify({ status: 'PASS', source_revision: process.env.GITHUB_SHA ?? null, checks, javascript_errors: errors, expected_offline_failures: failures, visual_review: 'Screenshots generated; human/model image inspection is separately required', production_ready: false }, null, 2) + '\n'); console.log(JSON.stringify({ browser: 'PASS', checks: checks.length }));
} catch (error) {
 writeFileSync('reports/browser/results.json', JSON.stringify({ status: 'FAIL', source_revision: process.env.GITHUB_SHA ?? null, checks, error: error.message, javascript_errors: errors, failures }, null, 2) + '\n'); throw error;
